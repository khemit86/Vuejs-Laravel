    ...
    <div>
        ...
        <section class="contener" slot="adminbody" style="margin-left:20%;margin-right:1.5%">
            <div class="card card-info">
              <div class="card-header">
                <h3 class="card-title" style="float:left">Add job</h3>
              ...
              <form class="form-horizontal" @submit.prevent="submit" ref="frm_job" id="frm_job" enctype="multipart/form-data">
                ...
                <div class="card-body">
                  ...
                  <div class="form-group row">
                    <label for="inputEmail3" class="col-sm-2 col-form-label">Post Job</label>
                    <div class="col-sm-10">
                      <select class="form-control" name="post_job_as" id="post_job_as" v-model="post_job_as" @change="choosePostJobAs">
                        <option value="" selected="selected">Select Post Job As*</option>
                        <option value="1">Employer</option>
                        <option value="2">Admin</option>                      
                     </select>
                     <span class="help-block" style="color:red" id="post_job_as_msg"></span>
                    <div class="help-block" style="color:red"  v-if="laravelErrors && laravelErrors.post_job_as">
                        <span>{{ laravelErrors.post_job_as[0] }}</span>
                    </div>
                    </div>
                  </div>
                  ...
                  <div id="adminContainer" style="display:none">
                       <div class="form-group row">
                        <label for="inputEmail3" class="col-sm-2 col-form-label">Plan Types</label>
                        <div class="col-sm-10">
                        <select class="form-control" name="plan_type" id="plan_type" v-model="plan_type" @change="choosePlan">
                            <option value="">Select Plan Types</option>
                            <option value="1">Singe job With Addon</option>
                            <option value="2">Addon Without Single Job</option> 
                            <option value="3">Subscription With Addon</option>
                            <option value="4">Subscription Without Addon</option>                                             
                        </select>
                        <span class="help-block" style="color:red" id="plan_type_msg"></span>
                        </div>
                    </div>

                    <div class="form-group row" id="subcriptionPlanDiv" style="display:none">
                        <label for="inputEmail3" class="col-sm-2 col-form-label">Subscription Plan</label>
                        <div class="col-sm-10">
                        <select class="form-control" name="subscription_plan_id" id="subscription_plan_id" v-model="subscription_plan_id">
                            <option value="">Select Subscription Plan</option>
                             <option v-for="value in dataSubscriptions" :key="value.id" :value="value.id">{{ value.title }}</option>                                   
                        </select>
                         <span class="help-block" style="color:red" id="subscription_plan_msg"></span>
                        </div>
                    </div>

                    <div class="form-group row" id="addonType" style="display:none">
                        <label for="inputEmail3" class="col-sm-2 col-form-label">Addon Types</label>
                        <div class="col-sm-10">
                        <select class="form-control" name="addon_id" id="addon_id" v-model="addon_id">
                            <option value="">Select Addon Types</option>
                            <option v-for="value in dataAddons" :key="value.id" :value="value.id">{{ value.title }}</option>
                        </select>
                        <span class="help-block" style="color:red" id="addon_id_msg"></span>
                        </div>
                    </div>
                  </div>

                  <div id="employerContainer" style="display:none">
                    <div class="form-group row">
                        <label for="inputEmail3" class="col-sm-2 col-form-label">Select Employer</label>
                        <div class="col-sm-10">
                         <div class="dropdown select_employeded">
                             <input type="hidden" id="employer_id" name="employer_id"/>
                            <input @keyup="getEmployer" v-if="Object.keys(selectedItem).length === 0" ref="dropdowninput" v-model.trim="inputValue" class="dropdown-input form-control" type="text" placeholder="Find Employer" />
                        ...
                        <span class="help-block" style="color:red" id="employer_id_msg"></span>
                        </div>
                    </div>

                    <div class="form-group row" id="userPlans" style="display:none">
                        <label for="inputEmail3" class="col-sm-2 col-form-label">Employer Plans</label>
                        <div class="col-sm-10">
                        <select class="form-control" name="user_subscription_plan_id" id="user_subscription_plan_id" v-model="user_subscription_plan_id">
                            <option value="">Select Employer Subscription Plan</option>
                             <option v-for="value in dataUserSubscriptions" :key="value.id" :value="value.id">{{ value.plan_title }}</option>                                   
                        </select>
                        <span class="help-block" style="color:red" id="user_subscription_plan_id_msg"></span>
                        </div>
                    </div>
        ...
        <div class="question-collapse-main questionContainer col-sm-10">
            ...
            <div  class="form-group row">
             ...
             <div class="question-collapse-main questionContainerRec col-sm-10">
                             <div class="panel-group driving-license-settings" id="accordion">
            ...
            <div class="card-footer">
              <button type="submit" name="btnSubmit" id="btnSubmit" class="btn btn-success">Save Job</button>
            </div>
            <!-- /.card-footer -->
          </form>
            </div>
...
<script>
     export default {
           data()
            ...
            methods:
            {
                choosePostJobAs(event)
                {
                  var postJobAs = event.target.value;
                  if(postJobAs == 1)
                  {
                      $('#adminContainer').hide();
                  ...
                  else
                  {
                   ...
                   this.addon_id = '';
                   this.user_subscription_plan_id= '',
                   this.plan_type = '';
                   $('#subscription_plan_id').val('');
                   $('#addon_id').val(''); 
                   $('#employer_id').val('');
                   $('#user_subscription_plan_id').val(''); 
                  $('#plan_type').val(''); 
                ...
                choosePlan(event)
                {
                    ...
                    axios.get('/api/admin/job/getAddon?type='+planType).then(response => {
                        if(response.data.status == 200)
                        {
                            this.dataAddons = response.data.addon;
                            this.dataSubscriptions = response.data.subscription;                               
                    ...
                    {
                         $('#subcriptionPlanDiv').show();
                    }
                    else
                    {
                         $('#subcriptionPlanDiv').hide();
                    }
                   this.subscription_plan_id = '';
                   this.addon_id = '';
                   $('#subscription_plan_id').val('');
                ...
                {
                    console.log(theItem)
                    $('#employer_id').val(theItem.id)
                    this.selectedItem = theItem 
                    this.inputValue = ''
                    this.$emit('on-item-selected', theItem)
                    axios.get('/api/admin/job/getSubscriptionplans?userid='+theItem.id).then( response =>  
                    {
                       console.log(response.data)
                       if(response.data != 0)
                       {
                         this.dataUserSubscriptions = response.data;
                         $('#userPlans').show();
                       }
                       else
                       {
                         $('#userPlans').hide();
                         this.flashMessage.error({
                                    message: 'Employer dont have any active plan',                            
                                });
                       }
                ...
                getEmployer(event) 
                {
                    ...
                    axios.get('/api/admin/job/getEmployers?search='+search).then( response => 
              ...
              {
                var valid = 1;
                var postJobAs = $('#post_job_as').val();
                var employerId = $('#employer_id').val();
                var dataUserSubscriptionPlanId = $('#user_subscription_plan_id').val();
                var planType = $('#plan_type').val();
                var addonId = $('#addon_id').val();
                var subscriptionPlanId = $('#subscription_plan_id').val();
                if(postJobAs == '')
                {
                  valid = 0;
                  $('#post_job_as_msg').text('Please select post job as!');
                }
                else
                {
                   $('#post_job_as_msg').text('')
                   if(postJobAs == 1)
                   {
                       if(employerId == '')
                       {
                          valid = 0;
                          $('#employer_id_msg').text('Please select employer!');
                       }
                       else
                       {
                          $('#employer_id_msg').text('');
                          if(dataUserSubscriptionPlanId == '')
                          {
                             $('#user_subscription_plan_id_msg').text('Please select employer plan!');
                             valid = 0;
                          }
                          else
                          {
                             $('#user_subscription_plan_id_msg').text('');
                             valid = 1;
                          }
                       }
                   }
                   else if(postJobAs == 2)
                   {
                      ...
                      {
                         valid = 0;
                         $('#plan_type_msg').text('Please select plan type!');
                      }
                     else
                     {
                        $('#plan_type_msg').text('');
                        if(planType == 1 || planType == 2)
                        {
                           ...
                           {
                              valid = 0;
                              $('#addon_id_msg').text('Please select an addon!')
                           }
                           else
                           {
                             $('#addon_id_msg').text('')
                        ...
                        else if(planType == 3)
                        {
                           ...
                           else
                           {
                             ...
                             {
                                $('#subscription_plan_msg').text('Please select an addon!');
                                valid = 0;
                             }
                             else
                             {
                                $('#subscription_plan_msg').text('')
                                valid = 1;
                             }
                           }
                        }
                        else if(planType == 4)
                        {
                            if(subscriptionPlanId == '')
                             {
                                $('#subscription_plan_id_msg').text('Please select an addon!');
                                valid = 0;
                             }
                             else
                             {
                                $('#subscription_plan_id_msg').text('')
                                valid = 1;
                             }
                        }
                     }
                ...
                if(stepValidationOne == 1 && stepValidationTwo == 1 && valid == 1)
                {
                    let formData = new FormData(this.$refs.frm_job);                    
                    axios.post('/api/admin/job/saveJob',formData).then(response => {
